    ...
    activation as (
        ...
        FROM (
            select 
            row_number() over(partition by short_url order by donation_created_wib_tableau desc) as row,
            split(is_activated_agent_name, chr(34))[SAFE_OFFSET(1)] as activated_agent_name,
            short_url
            from `kitabisa-data-team.data_mart.dt_campaign_activation_non_partnership`
            where donation_created_wib_tableau >= "2020-01-01"
