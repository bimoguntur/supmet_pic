...
ads_donation_1 as (
    ...
    on trim(ads_permalink.ad_name) = trim(donation_ga.utm_ad_name) 
        and date(ads_permalink.date_ads) = date(donation_ga.verified_day)
),
-- tbl_proj
project_1 as (
    Select
...
ads_donation_project_1 as (
    ...
    from ads_donation_1 full outer join project_details 
    on ads_donation_1.url_campaign = project_details.url_proj
),
