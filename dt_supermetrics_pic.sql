SELECT 
sum(gdv) as gdv, sum(trx) as trx, sum(cost) as cost 
FROM (
with ads_donation_project_ngo_list_1 as (
    with ads_donation_project_1 as (
        with ads_donation_1 as 
        (
            ---table ads_2, join with donation_1 = ads_donation_1
            with ads_2 as 
            (
                ---query ads_2
                -- tbl_ads_1 (union) & permalink
                with ads_1 as ( 
                    SELECT
                        date as date_ads,
		    ...
		    preview_link
                    ...
                    FROM ads_1 
                    left join ads_link on ads_1.ad_name_ads = ads_link.ad_name
                group by 1,2,3,4,5,6,7,8
            ),
            --- table donation_1, join with ads_2 = ads_donation_1
            donation_1 as 
            (
                SELECT
                sum(amount) as gdv,
	...
	-- tbl project_1 (join with ads_donation_1) = ads_donation_project_1
        project_1 as 
        (
            Select
	    ...
	    preview_link
            ...
            from ads_donation_1 left join project_1
            on ads_donation_1.url_campaign = project_1.url_proj
    ),
    region_ngo_id_2 as
    (
        Select *
    ...
    -- Query ads_donation_project_ngo_list_1
    Select
